...
// === V2 字段扩展 ===
const V2_DEFAULTS = { impact: 5, dependencies: [], monthlyCost: 0, costNotes: '', decisionLog: '' };
const IMPACTS = { 1: 9, 2: 7, 3: 6, 4: 5, 5: 4, 6: 3, 7: 3, 8: 8, 9: 7, 10: 6, 11: 5, 12: 4, 13: 8, 14: 6, 15: 4, 16: 2, 17: 3, 18: 9, 19: 8, 20: 5, 21: 7, 22: 6, 23: 8, 24: 5, 25: 4, 26: 4, 27: 5, 28: 4, 29: 3 };
const DEPS = { 2: [1], 3: [2], 4: [2], 5: [1], 6: [1], 7: [1], 9: [8], 10: [9], 11: [9], 12: [11], 14: [13], 15: [14], 24: [21], 25: [23], 26: [25], 27: [4], 28: [27], 29: [1] };
const COSTS = { 1: 500, 2: 800, 3: 600, 4: 400, 5: 100, 8: 300, 9: 400, 10: 500, 12: 200, 13: 200, 14: 300, 15: 800, 18: 100, 20: 150 };
DATA.forEach(d => {
    Object.assign(d, V2_DEFAULTS);
    if (IMPACTS[d.id]) d.impact = IMPACTS[d.id];
    if (DEPS[d.id]) d.dependencies = DEPS[d.id];
    if (COSTS[d.id]) d.monthlyCost = COSTS[d.id];
});

// === 技术分配更新：Copilot Studio 优先 + 业务数据在客户 CDP DB ===
const TECH_UPDATES = {
    1: { techs: [{ name: 'Copilot Studio', pct: 45 }, { name: 'NL2SQL/AI 能力集成', pct: 30 }, { name: 'API 集成（CDP 数据库）', pct: 20 }, { name: 'Dataverse（配置）', pct: 5 }], techAnalysis: 'Copilot Studio 作为主交互平台。NL2SQL 能力集成（取决于客户是否已有服务）。通过 API 连接客户 CDP 数据库查询业务数据。Dataverse 仅存知识库配置和模板。' },
    2: { techs: [{ name: 'Copilot Studio', pct: 40 }, { name: 'NL2SQL/AI 能力集成', pct: 30 }, { name: 'API 集成（CDP 数据库）', pct: 25 }, { name: 'Dataverse（配置）', pct: 5 }], techAnalysis: '扩展 schema 覆盖。Copilot Studio 做 Agent 编排。NL2SQL 扩展。API 对接更多 CDP 报表数据源。' },
    3: { techs: [{ name: 'Copilot Studio', pct: 35 }, { name: 'NL2SQL/AI 能力集成', pct: 30 }, { name: 'API 集成（CDP 数据库）', pct: 20 }, { name: 'Power BI Embedded', pct: 15 }], techAnalysis: 'Copilot Studio Agent 做语义入口。AI 能力做特征识别。API 查客户 CDP 数据库做画像聚合。可视化用 Power BI。' },
    4: { techs: [{ name: 'Copilot Studio', pct: 40 }, { name: 'NL2SQL/AI 能力集成', pct: 35 }, { name: 'API 集成（CDP 数据库）', pct: 25 }], techAnalysis: 'Copilot Studio Agent 做复盘交互。AI 能力做数据分析。API 拉取客户 CDP 数据库中的营销活动数据。' },
    5: { techs: [{ name: 'Copilot Studio', pct: 55 }, { name: 'NL2SQL/AI 能力集成', pct: 25 }, { name: 'Dataverse（配置）', pct: 20 }], techAnalysis: '主要在 Copilot Studio 内补充 Topic 和意图规则。AI 辅助意图识别。Dataverse 存未识别意图日志。' },
    6: { techs: [{ name: 'Copilot Studio', pct: 35 }, { name: 'NL2SQL/AI 能力集成', pct: 35 }, { name: 'API 集成（CDP 数据库）', pct: 30 }], techAnalysis: '扩展为多活动对比分析。Copilot Studio 做选择交互。AI 做跨活动分析。API 拉取多活动数据。' },
    7: { techs: [{ name: 'Copilot Studio', pct: 45 }, { name: 'NL2SQL/AI 能力集成', pct: 35 }, { name: 'API 集成（CDP 数据库）', pct: 20 }], techAnalysis: 'Copilot Studio Agent 查个人会员数据。AI 生成画像描述。数据从客户 CDP 数据库获取。' },
    8: { techs: [{ name: 'Copilot Studio', pct: 45 }, { name: 'NL2SQL/AI 能力集成', pct: 40 }, { name: 'Dataverse（Prompt 模板）', pct: 15 }], techAnalysis: 'Copilot Studio Agent 做文案交互入口。AI 能力做文案生成。Prompt 模板存 Dataverse。人群特征从 CDP DB 通过 API 获取。' },
    9: { techs: [{ name: 'Copilot Studio', pct: 35 }, { name: 'NL2SQL/AI 能力集成', pct: 25 }, { name: 'API 集成（CDP 接口）', pct: 35 }, { name: 'Dataverse（配置）', pct: 5 }], techAnalysis: 'Copilot Studio Agent 做对话式圈人。AI 能力解析圈人条件。核心依赖：CDP 系统的标签/行为筛选 API。' },
    10: { techs: [{ name: 'Copilot Studio', pct: 30 }, { name: 'NL2SQL/AI 能力集成', pct: 25 }, { name: 'API 集成（CDP 接口）', pct: 40 }, { name: 'Dataverse（配置）', pct: 5 }], techAnalysis: 'Copilot Studio 做目标解析。AI 生成画布结构。核心在 API 调 CDP 画布创建接口——API 开放度是最大风险。' },
    11: { techs: [{ name: 'Copilot Studio', pct: 35 }, { name: 'NL2SQL/AI 能力集成', pct: 35 }, { name: 'API 集成（CDP 接口）', pct: 25 }, { name: 'Dataverse（配置）', pct: 5 }], techAnalysis: '升级语义理解。Copilot Studio + AI 能力做深层语义。API 对接 CDP 行为数据。' },
    12: { techs: [{ name: 'Copilot Studio', pct: 15 }, { name: 'AI 图片生成服务', pct: 50 }, { name: '自定义开发', pct: 25 }, { name: 'Dataverse（模板）', pct: 10 }], techAnalysis: 'Copilot Studio 做生成入口。核心是 AI 图片生成服务。需自定义品牌模板管理和审核流程。' },
    13: { techs: [{ name: 'Copilot Studio', pct: 50 }, { name: 'NL2SQL/AI 能力集成', pct: 30 }, { name: 'Dataverse（知识库）', pct: 20 }], techAnalysis: 'Copilot Studio Agent 做策略问答——最擅长的场景。AI 能力做策略生成。知识库存 Dataverse。' },
    14: { techs: [{ name: 'Copilot Studio', pct: 30 }, { name: 'NL2SQL/AI 能力集成', pct: 30 }, { name: 'API 集成（CDP 数据库）', pct: 25 }, { name: '自定义策略引擎', pct: 15 }], techAnalysis: 'Copilot Studio 做交互层。AI 做建议生成。需 API 从 CDP 数据库拉历史营销数据。策略引擎需自定义。' },
    15: { techs: [{ name: 'Copilot Studio', pct: 20 }, { name: 'NL2SQL/AI 能力集成', pct: 30 }, { name: '自定义策略引擎', pct: 35 }, { name: 'API 集成（CDP 数据库）', pct: 15 }], techAnalysis: '最复杂模块。核心是自定义策略引擎 + AI 效果归因分析。需要大量 CDP DB 历史数据。' },
    16: { techs: [{ name: 'Copilot Studio', pct: 80 }, { name: 'Dataverse（知识库）', pct: 20 }], techAnalysis: '纯 Copilot Studio 知识库功能，导入操作手册即可。最简单的功能点。' },
    17: { techs: [{ name: 'Copilot Studio', pct: 45 }, { name: 'NL2SQL/AI 能力集成', pct: 30 }, { name: 'API 集成（CDP 数据库）', pct: 25 }], techAnalysis: '复用智能问数 NL2SQL 能力。Copilot Studio Agent 做问询交互。API 查 CDP 数据库指标。' },
    18: { techs: [{ name: 'Copilot Studio', pct: 25 }, { name: 'Dataverse', pct: 40 }, { name: 'Power Apps', pct: 35 }], techAnalysis: '知识库是配置数据，存 Dataverse。Power Apps 做管理界面。Copilot Studio 知识源直连。' },
    19: { techs: [{ name: 'Copilot Studio', pct: 15 }, { name: 'Dataverse', pct: 45 }, { name: 'Power Apps', pct: 40 }], techAnalysis: 'Prompt 模板是配置数据，存 Dataverse。Power Apps 做 CRUD。' },
    20: { techs: [{ name: 'Copilot Studio', pct: 20 }, { name: 'Dataverse', pct: 35 }, { name: 'Power Apps', pct: 25 }, { name: 'Power BI', pct: 20 }], techAnalysis: '使用日志存 Dataverse。Power Apps/BI 做统计看板。' },
    21: { techs: [{ name: 'Copilot Studio', pct: 30 }, { name: 'Dataverse', pct: 30 }, { name: 'Power Apps', pct: 25 }, { name: '自定义规则引擎', pct: 15 }], techAnalysis: '策略规则存 Dataverse。Power Apps 做配置。Copilot Studio Agent 调用规则解读。' },
    22: { techs: [{ name: 'Copilot Studio', pct: 30 }, { name: 'Dataverse', pct: 30 }, { name: 'Power Apps', pct: 25 }, { name: '自定义规则引擎', pct: 15 }], techAnalysis: '与核心数据解读相同结构，针对画像维度。' },
    23: { techs: [{ name: 'Copilot Studio', pct: 70 }, { name: '自定义开发（iframe）', pct: 25 }, { name: 'Dataverse（配置）', pct: 5 }], techAnalysis: '核心就是 Copilot Studio Web Channel + iframe 嵌入。最直接体现 Copilot Studio 价值。' },
    24: { techs: [{ name: 'Copilot Studio', pct: 25 }, { name: 'Dataverse', pct: 30 }, { name: '自定义规则引擎', pct: 30 }, { name: 'NL2SQL/AI 能力集成', pct: 15 }], techAnalysis: 'V1.0 基础上扩展更多维度。Copilot Studio 做 Agent 运行时。' },
    25: { techs: [{ name: 'Copilot Studio', pct: 50 }, { name: '自定义开发', pct: 35 }, { name: 'Power Apps', pct: 15 }], techAnalysis: '从 iframe 升级为独立 Tab。核心仍是 Copilot Studio。' },
    26: { techs: [{ name: 'Copilot Studio', pct: 60 }, { name: 'Dataverse（模板）', pct: 25 }, { name: '自定义开发', pct: 15 }], techAnalysis: '核心在 Copilot Studio Topic 设计——六步引导流程。' },
    27: { techs: [{ name: 'Copilot Studio', pct: 25 }, { name: 'Dataverse', pct: 30 }, { name: '自定义规则引擎', pct: 25 }, { name: 'NL2SQL/AI 能力集成', pct: 20 }], techAnalysis: '为复盘 Agent 提供解读规则配置。' },
    28: { techs: [{ name: 'Copilot Studio', pct: 25 }, { name: 'Dataverse', pct: 30 }, { name: '自定义规则引擎', pct: 25 }, { name: 'NL2SQL/AI 能力集成', pct: 20 }], techAnalysis: '与复盘策略类似结构。' },
    29: { techs: [{ name: 'Copilot Studio', pct: 40 }, { name: '自定义开发', pct: 40 }, { name: 'NL2SQL/AI 能力集成', pct: 20 }], techAnalysis: '基于反馈升级交互框架。Copilot Studio 做核心平台。' },
};
Object.entries(TECH_UPDATES).forEach(([id, update]) => {
    const d = DATA.find(x => x.id === parseInt(id));
    if (d) { d.techs = update.techs; d.techAnalysis = update.techAnalysis; }
});
...
const PLANS = [
    {
        id: 'plan-a',
        name: '方案一：优先级驱动 — Copilot Studio 快速验证',
        tag: '推荐',
        summary: '聚焦 P0 高优先级功能，先打通核心产品验证 Copilot Studio + NL2SQL 价值。',
        strategy: '选取智能问数（P0）+ AI基础后台必要组件，用 Copilot Studio 做主平台最小化验证。营销助手/策略放第二批次。',
        coreFeatures: ['智能问数一期（会员概览数据智能解读）— P0 核心验证项', '知识库管理 — AI 基础能力底座', 'CDP AI 助手嵌入 — 最直接体现 Copilot Studio 价值', '策略中心 — 数据解读规则配置'],
        phases: [
    ...
    },
    {
        ...
        tag: null,
        summary: '全量评估所有功能点，四大模块并行推进 V1.0，一次性交付完整 AI 能力底座。',
        strategy: '同时启动智能问数、营销助手、营销策略、AI基础后台的 V1.0 功能。',
        coreFeatures: ['智能问数一期 — 数据查询能力', 'AI生成营销文案 — 内容生产能力', '通用营销方案策划 — 策略推荐能力', '知识库 + 提示词 + 策略中心 — AI 基础设施'],
        phases: [
